...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  while(1) {
    ...
    if(NETSTACK_ROUTING.node_is_reachable() &&
      ...
      /*
       * This block only runs on node ID 2.
       * Node 3 has the same firmware, but it does not send application messages.
       * Node 3 only helps as a network neighbor/relay.
       */
      if(node_id == 2) {
    ...
    /* Add some jitter: next send happens around 9-11 seconds */
